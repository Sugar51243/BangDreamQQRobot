...
async def sreachCard(bot, msg, cardID):
    ...
    try:
        ...
        message += MessageChain(f'{skill["simpleDescription"][0]} ({skill["simpleDescription"][3]})\n')
    except:
        message += MessageChain(["获取失败\n"])
...
#Just get the image from bestdori by id
...
#Random a vaild card id
async def randomCardID(bot, msg):
    
    try:
        url = "https://bestdori.com/api/costumes/all.0.json"
        data = requestData(url)
    except:
        message = MessageChain(["服务器网络连接出错"])
        await sendMessage.replyMsgToGroup(bot, msg, message)
