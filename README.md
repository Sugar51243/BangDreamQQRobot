# Main
 A NapCat QQ Robot usage to Bang Dream Information Searching via Bestdori.
一个基于NcatBot/NapCat的类茨菇QQ机器人。
目前支持功能: `Bang Dream` | `YGO群赛` | `BiliBili视频获取`

# Bang Dream
Bang Dream, 以下简称邦邦。
目前邦邦查谱类功能为此机器人主要功能。主要目的是快速查询自制谱及官谱谱面图片及基础信息。主要信息来源为Bestdori API。
...
信息将包括: `谱面名称` `谱面ID` `谱面难度` `谱面等级` `BPM` `物量` `时长` `所属服务器` `谱师` `点赞数` `谱面`
...
信息将包括: `谱面名称` `谱面ID` `谱面难度` `谱面等级` `BPM` `物量` `时长` `所属服务器` `所属乐队` `谱面`
    **所有信息皆由Bestori提供**
...
信息将包括: `角色名称` `所处乐团` `卡片名称` `卡片ID` `星级` `属性` `等级上限` `技能` `数据` `卡面`
    **所有信息皆由Bestori提供**
...
信息将包括: `用户名称` `用户等级` `用户ID` `账号简介` `综合力` `通关状况`
    **所有信息皆由Bestori提供**
...
# YGO群赛
游戏王群赛, 以下简称YGO。
YGO类功能为此机器人的副功能。主要目的是快速开启群赛，并公开匹配选手。
...
# 部署
本程序依赖 `NcatBot` `PIL` `httpx` `eyed3` 运行，请自行依次于环境补完运行库。
